...
func (c completedConfig) New() (*WardleServer, error) {
	...
	for _, apiGroup := range apiGroups {
		if len(apiGroup.PrioritizedVersions[0].Group) > 0 {
			if err := s.GenericAPIServer.InstallAPIGroup(apiGroup); err != nil {
				return nil, err
			}
		} else {
			// fornax want to install some legacy k8s resources, so, have to add this,
			// TODO, refactor apiserver-runtime to decouple apiserver-runtime builder with sample-api server
			if err := s.GenericAPIServer.InstallLegacyAPIGroup(server.DefaultLegacyAPIPrefix, apiGroup); err != nil {
				return nil, err
			}
		}
	}
