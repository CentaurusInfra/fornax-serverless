...
import (
	...
	"k8s.io/klog/v2"
	kubletevents "k8s.io/kubernetes/pkg/kubelet/events"
)
	...
	if len(cm.cgroupRoot) > 0 {
		go func() {
			for {
				...
				if err == nil {
					cm.recorder.Event(nodeRef, v1.EventTypeNormal, kubletevents.SuccessfulNodeAllocatableEnforcement, "Updated Node Allocatable limit across pods")
					return
				}
...
func GetCgroupConfig(rl v1.ResourceList) *ResourceConfig {
	...
	if q, exists := rl[ResourcePID]; exists {
		val := q.Value()
		rc.PidsLimit = &val
	}
...
func (cm *ContainerManagerImpl) GetNodeAllocatableReservation() v1.ResourceList {
	...
	for k := range cm.capacity {
		...
		if cm.NodeConfig.EvictionReservation != nil {
			value.Add(cm.NodeConfig.EvictionReservation[k])
		}
		if !value.IsZero() {
			result[k] = *value
		}
