...
var (
	...
	_InMemoryStores      = map[string]storage.Interface{}
)

// NewMemoryStore return a singleton storage.Interface for a prefix/groupResource combination
func NewMemoryStore(newFunc func() runtime.Object, prefix string, groupResource schema.GroupResource, pagingEnabled bool) storage.Interface {
	...
	key := fmt.Sprintf("%s/%s", prefix, groupResource.String())
	if si, found := _InMemoryStores[key]; found {
		return si
	} else {
		klog.InfoS("New in memory store for", "resource", key)
		si = &memoryStore{
