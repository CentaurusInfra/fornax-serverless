...
func GetContainer(pid int) (string, error) {
	cgs, err := libcontainercgroups.ParseCgroupFile(fmt.Sprintf("/proc/%d/cgroup", pid))
	if err != nil {
		return "", err
	}
	...
	if libcontainercgroups.IsCgroup2UnifiedMode() {
		c, found := cgs[""]
		if !found {
			return "", libcontainercgroups.NewNotFoundError("unified")
		}
		return c, nil
	}
	...
	cpu, found := cgs["cpu"]
	if !found {
		return "", libcontainercgroups.NewNotFoundError("cpu")
	}
	memory, found := cgs["memory"]
	if !found {
		return "", libcontainercgroups.NewNotFoundError("memory")
	}
