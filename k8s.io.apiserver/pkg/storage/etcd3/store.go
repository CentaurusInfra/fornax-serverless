...
func (s *store) Get(ctx context.Context, key string, opts storage.GetOptions, out runtime.Object) error {
	...
	res := decode(s.codec, s.versioner, data, out, kv.ModRevision)
	klog.InfoS("GWJ get a object", "key", key, "obj", out)
	return res
}
...
func (s *store) Create(ctx context.Context, key string, obj, out runtime.Object, ttl uint64) error {
	...
	if out != nil {
		...
		resp := decode(s.codec, s.versioner, data, out, putResp.Header.Revision)
		klog.InfoS("GWJ create a object", "key", key, "obj", out)
		return resp
	}
	return nil
}
...
func (s *store) GetList(ctx context.Context, key string, opts storage.ListOptions, listObj runtime.Object) error {
	...
	klog.InfoS("GWJ query list of obj", "withRV", withRev, "returnRV", returnedRV, "fromRV", fromRV)
	...
	if hasMore {
		...
		resp := s.versioner.UpdateList(listObj, uint64(returnedRV), next, remainingItemCount)
		klog.InfoS("GWJ return list of obj", "list", listObj, "returnRV", returnedRV)
		return resp
	}

	// no continuation
	resp := s.versioner.UpdateList(listObj, uint64(returnedRV), "", nil)
	klog.InfoS("GWJ return list of obj", "list", listObj, "returnRV", returnedRV)
	return resp
}
